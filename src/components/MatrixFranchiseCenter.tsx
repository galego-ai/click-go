...
export default function MatrixFranchiseCenter(){
 ...
 async function generateAccess(){if(!selected)return;const why=reason(`Gerar ou redefinir acesso administrativo de ${selected.trade_name}`);if(!why){setMsg('Operação cancelada: a justificativa é obrigatória.');return}const chosen=window.prompt('Senha temporária (mínimo 8 caracteres com letras e números). Deixe em branco para gerar automaticamente.','');if(chosen===null)return;setBusy('access');setTemp(null);const body:any={action:'generate',franchise_id:selected.id,reason:why};if(chosen.trim())body.temporary_password=chosen.trim();const{data,error}=await supabase.functions.invoke('franchise-temp-password',{body});setBusy('');if(error||data?.error){setMsg(String(data?.error||error?.message||'Falha ao gerar acesso.'));return}setTemp({franchise:selected.trade_name,email:data.email,password:data.temporary_password});setMsg('Acesso administrativo preparado com senha temporária.')}
