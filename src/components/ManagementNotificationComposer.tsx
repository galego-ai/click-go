  ...
  return <div style={{display:'grid',gap:14,marginBottom:18}}>
    <section style={card}>
      ...
      <div style={{display:'grid',gap:12}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,flexWrap:'wrap'}}><div style={{fontSize:12,color:'#666'}}>{mode==='all'?`Destino: ${appLabel(targetApp)} · todos com app ativo`:`Destino: ${selectedCount} pessoa(s) escolhida(s)`}</div><button onClick={send} disabled={sending||loading} style={{...yellow,opacity:sending||loading?0.65:1}}>{sending?'Enviando…':'🔔 Disparar notificação'}</button></div>
      </div>
