...
export default function CityManager(){
 ...
 const[name,setName]=useState(''),[state,setState]=useState('GO'),[createReason,setCreateReason]=useState(''),[query,setQuery]=useState(''),[msg,setMsg]=useState('')
 ...
 async function add(e:React.FormEvent){
  e.preventDefault()
  if(!createReason.trim()){setMsg('Informe a justificativa do cadastro da cidade.');return}
  setBusy('new')
  const{error}=await supabase.rpc('matrix_create_city',{p_name:name.trim(),p_state:state.trim().toUpperCase(),p_reason:createReason.trim()})
  setBusy('')
  if(error){setMsg(error.message);return}
  setName('');setCreateReason('');setMsg('Cidade cadastrada, auditada e pronta para receber um território.');void load()
 }
 async function toggle(c:City){
  const why=(reason[c.id]||'').trim();if(!why){setMsg('Informe a justificativa antes de bloquear ou ativar uma cidade.');return}
  setBusy(c.id)
  const{error}=await supabase.rpc('matrix_set_city_active',{p_city_id:c.id,p_active:!c.active,p_reason:why})
  setBusy('');setMsg(error?error.message:`${c.name} ${c.active?'bloqueada':'ativada'} e registrada na auditoria.`);if(!error){setReason(v=>({...v,[c.id]:''}));void load()}
 }
 ...
 async function removeCity(c:City){
  if(ownerMap.has(c.id)){setMsg('Remova primeiro o vínculo territorial da cidade. A Matriz não pode excluir uma cidade que pertence a uma franquia.');return}
  const why=(reason[c.id]||'').trim();if(!why){setMsg('Informe a justificativa antes de excluir a cidade.');return}
  if(!confirm(`Excluir definitivamente ${c.name}/${c.state}? Só cidades sem qualquer histórico ou configuração podem ser apagadas.`))return
  setBusy(c.id);const{error}=await supabase.rpc('matrix_delete_city',{p_city_id:c.id,p_reason:why});setBusy('');setMsg(error?error.message:'Cidade excluída e registrada na auditoria.');if(!error){setReason(v=>({...v,[c.id]:''}));void load()}
 }

 return <>
  <div className="card"><div className="section-heading"><div><div className="eyebrow">Cadastro territorial</div><h2>Adicionar cidade à rede</h2><p className="subtitle">Cadastrar uma cidade não dá exclusividade automaticamente. O vínculo com a franquia é feito logo abaixo.</p></div><MapPin size={22}/></div><form onSubmit={add}><div className="form-grid"><div className="field"><label>Cidade</label><input className="input" value={name} onChange={e=>setName(e.target.value)} required placeholder="Ex.: Uruaçu"/></div><div className="field"><label>UF</label><input className="input" maxLength={2} value={state} onChange={e=>setState(e.target.value)} required/></div></div><div className="field" style={{marginTop:12}}><label>Justificativa do cadastro</label><textarea className="input" value={createReason} onChange={e=>setCreateReason(e.target.value)} required placeholder="Ex.: nova praça aprovada pela Matriz para expansão da rede"/></div><div className="toolbar" style={{marginTop:14}}><button className="button" disabled={busy==='new'}><Plus size={15}/>{busy==='new'?'Salvando...':'Cadastrar cidade'}</button></div></form></div>
