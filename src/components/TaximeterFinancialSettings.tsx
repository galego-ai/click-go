...
export default function TaximeterFinancialSettings({network=false}:{network?:boolean}){
 ...
 return <section style={{...box,borderColor:'#574900',marginBottom:14}}>
  ...
  <div style={{display:'grid',gridTemplateColumns:'minmax(190px,1fr) minmax(150px,220px) auto',gap:10,alignItems:'end',marginTop:14}}>
   <label style={{display:'grid',gap:5,fontSize:12,color:'#9ca3af'}}>Modelo<select disabled={busy||(!network&&!data?.can_edit)} value={mode} onChange={e=>setMode(e.target.value as any)} style={input}><option value="none">Sem taxa</option><option value="fixed">Valor fixo por corrida</option><option value="percentage">Percentual da corrida</option></select></label>
   <label style={{display:'grid',gap:5,fontSize:12,color:'#9ca3af'}}>{mode==='percentage'?'Percentual (%)':'Valor (R$)'}<input disabled={busy||mode==='none'||(!network&&!data?.can_edit)} value={value} onChange={e=>setValue(e.target.value)} inputMode="decimal" style={input}/></label>
   <button disabled={busy||(!network&&!data?.can_edit)} onClick={save} style={{...btn,opacity:(busy||(!network&&!data?.can_edit))?.55:1}}>{busy?'Salvando…':'Salvar regra'}</button>
  </div>
