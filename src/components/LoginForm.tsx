...
export default function LoginForm(){
 ...
 async function login(e:React.FormEvent){
  ...
  if(profileError||!profile){await supabase.auth.signOut();setMsg('Não foi possível validar o perfil administrativo.');setBusy(false);return}
  ...
  if(role!=='super_admin'){await supabase.auth.signOut();setMsg('Este acesso não é de Super Admin.');setBusy(false);return}
  if(profile.active===false){await supabase.auth.signOut();setMsg('Este acesso administrativo está inativo.');setBusy(false);return}
