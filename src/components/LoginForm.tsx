...
export default function LoginForm(){
 ...
 async function recover(){if(!email.trim()){setMsg('Digite primeiro o e-mail da conta Super Admin.');return}setBusy(true);setMsg('Enviando recuperação...');const redirectTo=`${window.location.origin}/redefinir-senha`;const{error}=await supabase.auth.resetPasswordForEmail(email.trim(),{redirectTo});setBusy(false);setMsg(error?error.message:'Link de recuperação enviado. Abra o e-mail recebido para criar uma nova senha.')}
