...
export default function PassengerPage() {
  ...
  async function addPayment(e:FormEvent<HTMLFormElement>){
    ...
    if(methods.length)await supabase.from('passenger_payment_methods').update({is_default:false}).eq('passenger_id',profile.id)
  ...
  return <main style={{minHeight:'100vh',background:'#080808',color:'#f8fafc',padding:20}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'230px 1fr',gap:16,alignItems:'start'}}>
      ...
      <section style={box}><div className="eyebrow">{active}</div>
        ...
        {active==='Solicitar corrida'&&<PassengerRideRequest methods={methods} onRideCreated={()=>loadPassenger(profile.id)}/>} 
        ...
        {active==='Endereços favoritos'&&<><h2>Endereços favoritos</h2><p className="subtitle">Casa, trabalho e outros endereços serão adicionados quando a geocodificação por endereço estiver conectada.</p></>}
