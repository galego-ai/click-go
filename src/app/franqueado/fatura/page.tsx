...
import {CalendarDays, Copy, CreditCard, ExternalLink, QrCode, RefreshCw} from 'lucide-react'
...
export default function FranchiseInvoicePage(){
 ...
 async function load(){
  ...
  const ref=`${month}-01`
  const[b,c]=await Promise.all([supabase.rpc('get_franchise_billing_summary',{p_franchise_id:fid,p_reference_month:ref}),supabase.rpc('get_franchise_collection_state',{p_franchise_id:fid})])
  if(b.error||c.error){setMsg(b.error?.message||c.error?.message||'Não foi possível carregar a fatura.');setBusy(false);return}
  const current=b.data as Billing;setBill(current);setCollection(c.data as Collection);setPix(null);setCard(null)
