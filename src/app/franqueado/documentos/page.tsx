...
export default function DriverDocumentsPage(){
 ...
 async function decideDriver(driver:Driver,approved:boolean){
  setMsg('');const list=docsByDriver[driver.id]||[]
  if(approved){if(!list.length){setMsg('Este motorista ainda não enviou documentos. Analise os documentos antes de aprovar.');return}const pend=list.filter(d=>d.status!=='approved');if(pend.length){setMsg('Todos os documentos do motorista precisam estar aprovados antes da liberação.');return}}
  ...
  setBusy(true);const{data:{user}}=await supabase.auth.getUser();const payload=approved?{status:'approved',approved_at:new Date().toISOString(),approved_by:user?.id||null,rejection_reason:null,online:false}:{status:'rejected',approved_at:null,approved_by:user?.id||null,rejection_reason:reason,online:false};const{error}=await supabase.from('drivers').update(payload).eq('id',driver.id);setBusy(false);setMsg(error?error.message:(approved?'Motorista aprovado e liberado para operar.':'Motorista reprovado.'));if(!error)await load()
 }
 return <main style={{minHeight:'100vh',background:'#080808',color:'#f8fafc',padding:28}}>
  <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'center',marginBottom:22}}><div><div style={{color:'#ffd400',fontWeight:800,fontSize:12,textTransform:'uppercase'}}>Central do Franqueado</div><h1 style={{margin:'5px 0'}}>Aprovação de motoristas</h1><p style={{color:'#9ca3af',margin:0}}>Analise os documentos e depois aprove ou reprove o cadastro do motorista.</p></div><div style={{display:'flex',gap:8}}><Link href="/franqueado" style={{...btn,textDecoration:'none'}}>Voltar ao painel</Link><button style={btn} onClick={load}>{busy?'Atualizando...':'Atualizar'}</button></div></div>
  {msg&&<div style={{...box,borderColor:'#665600',color:'#ffe66b',marginBottom:16}}>{msg}</div>}
  <div style={{display:'grid',gap:16}}>{drivers.map(driver=>{const p=names[driver.id];const list=docsByDriver[driver.id]||[];const allApproved=list.length>0&&list.every(d=>d.status==='approved');return <section key={driver.id} style={{...box,borderColor:driver.status==='approved'?'#176b45':driver.status==='rejected'?'#6b1f1f':'#4b4b22'}}>
   <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}><div><h3 style={{margin:'0 0 4px'}}>{p?.full_name||'Motorista'}</h3><div style={{color:'#9ca3af',fontSize:13}}>{p?.phone||p?.email||driver.id}</div><div style={{marginTop:8}}>Status: <b>{driver.status}</b>{driver.rejection_reason&&<span style={{color:'#fca5a5'}}> · {driver.rejection_reason}</span>}</div></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{driver.status!=='approved'&&<button style={{...btn,opacity:allApproved?1:.55}} onClick={()=>decideDriver(driver,true)}>Aprovar motorista</button>}<button style={{...btn,background:'#3a1b1b',color:'#fff'}} onClick={()=>decideDriver(driver,false)}>Reprovar motorista</button></div></div>
   <div style={{marginTop:14,display:'grid',gap:8}}>{list.length?list.map(doc=><div key={doc.id} style={{background:'#0d0d0d',border:'1px solid #272727',borderRadius:12,padding:12,display:'grid',gridTemplateColumns:'1.4fr 1fr auto',gap:12,alignItems:'center'}}><div><b>{doc.document_type}</b><div style={{color:'#9ca3af',fontSize:12}}>{new Date(doc.created_at).toLocaleString('pt-BR')}</div></div><div><span style={{padding:'5px 9px',borderRadius:999,background:'#222'}}>{doc.status}</span>{doc.rejection_reason&&<div style={{color:'#fca5a5',fontSize:12,marginTop:6}}>{doc.rejection_reason}</div>}</div><div style={{display:'flex',gap:7,flexWrap:'wrap'}}><button style={{...btn,background:'#252525',color:'#fff'}} onClick={()=>openDoc(doc)}>Ver</button>{doc.status==='pending'&&<><button style={btn} onClick={()=>review(doc,true)}>Aprovar documento</button><button style={{...btn,background:'#3a1b1b',color:'#fff'}} onClick={()=>review(doc,false)}>Reprovar</button></>}</div></div>):<div style={{color:'#9ca3af'}}>Nenhum documento enviado ainda.</div>}</div>
