...
export default function DriverDocumentsPage(){
 const[docs,setDocs]=useState<Doc[]>([]),[profiles,setProfiles]=useState<Profile[]>([]),[drivers,setDrivers]=useState<Driver[]>([]),[msg,setMsg]=useState(''),[busy,setBusy]=useState(false),[franchise,setFranchise]=useState<FranchiseState|null>(null)
 ...
 async function load(){setBusy(true);setMsg('');try{
  ...
  const{data:f,error:fErr}=await supabase.from('franchises').select('id,trade_name,active,deleted_at,blocked_at').eq('id',me.franchise_id).maybeSingle();if(fErr)throw fErr
  setFranchise((f||null) as FranchiseState|null)
  if(!f){setDrivers([]);setDocs([]);setProfiles([]);throw new Error('Seu acesso aponta para uma franquia que não existe mais. O Super Admin precisa corrigir o vínculo em Franquias > Acessos dos franqueados.')}
  if(!f.active||f.deleted_at||f.blocked_at){setDrivers([]);setDocs([]);setProfiles([]);throw new Error(`A franquia “${f.trade_name}” está inativa, bloqueada ou excluída. O Super Admin precisa corrigir seu vínculo antes de aprovar motoristas.`)}
 ...
 async function decideDriver(driver:Driver,approved:boolean){
  ...
  if(approved){if(driver.status==='approved'){setMsg('Este motorista já está aprovado.');return}if(!p?.avatar_url){setMsg('Aprovação bloqueada: falta a foto real de perfil do motorista.');return}if(!list.length){setMsg('Aprovação bloqueada: o motorista ainda não enviou documentos.');return}if(!photoApproved){setMsg('Aprovação bloqueada: abra e aprove primeiro a “Foto real de perfil”.');return}if(list.some(d=>d.status!=='approved')){setMsg('Aprovação bloqueada: ainda existem documentos pendentes ou reprovados.');return}}
 ...
 return <main style={{minHeight:'100vh',background:'#080808',color:'#f8fafc',padding:28}}>
  <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'center',marginBottom:22}}><div><div style={{color:'#ffd400',fontWeight:800,fontSize:12,textTransform:'uppercase'}}>Central do Franqueado</div><h1 style={{margin:'5px 0'}}>Aprovação de motoristas</h1><p style={{color:'#9ca3af',margin:0}}>Fluxo: foto real → documentos → aprovação final do cadastro.</p></div><div style={{display:'flex',gap:8}}><Link href="/franqueado" style={{...btn,textDecoration:'none'}}>Voltar ao painel</Link><button style={btn} onClick={load}>{busy?'Atualizando...':'Atualizar'}</button></div></div>
  {franchise&&<div style={{...box,marginBottom:14,borderColor:franchiseOk?'#176b45':'#7f1d1d'}}><b>{franchise.trade_name}</b> · <span style={{color:franchiseOk?'#86efac':'#fca5a5'}}>{franchiseOk?'Franquia ativa — aprovações liberadas':'Franquia inativa/bloqueada/excluída — aprovações bloqueadas'}</span></div>}
  {msg&&<div style={{...box,borderColor:'#665600',color:'#ffe66b',marginBottom:16}}>{msg}</div>}
  <div style={{display:'grid',gap:16}}>{drivers.map(driver=>{const p=names[driver.id];const list=docsByDriver[driver.id]||[];const allApproved=list.length>0&&list.every(d=>d.status==='approved');const photoApproved=list.some(d=>d.document_type==='profile_photo'&&d.status==='approved');const canApprove=franchiseOk&&driver.status!=='approved'&&!!p?.avatar_url&&photoApproved&&allApproved;let blockReason='Pronto para aprovação final';if(driver.status==='approved')blockReason='Motorista já aprovado';else if(!p?.avatar_url)blockReason='Falta foto real de perfil';else if(!list.length)blockReason='Faltam documentos';else if(!photoApproved)blockReason='A foto real ainda não foi aprovada';else if(!allApproved)blockReason='Existem documentos pendentes/reprovados';return <section key={driver.id} style={{...box,borderColor:driver.status==='approved'?'#176b45':driver.status==='rejected'?'#6b1f1f':'#4b4b22'}}>
   <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}><div style={{display:'flex',gap:14,alignItems:'center'}}>{p?.avatar_url?<img src={p.avatar_url} alt="Foto real do motorista" style={{width:78,height:78,borderRadius:'50%',objectFit:'cover',border:'3px solid #ffd400'}}/>:<div style={{width:78,height:78,borderRadius:'50%',display:'grid',placeItems:'center',background:'#2a1717',border:'2px solid #7f1d1d',color:'#fca5a5',fontSize:11,textAlign:'center',padding:6}}>FOTO OBRIGATÓRIA AUSENTE</div>}<div><h3 style={{margin:'0 0 4px'}}>{p?.full_name||'Motorista'}</h3><div style={{color:'#9ca3af',fontSize:13}}>{p?.phone||p?.email||driver.id}</div><div style={{marginTop:8}}>Status: <b>{driver.status}</b>{driver.rejection_reason&&<span style={{color:'#fca5a5'}}> · {driver.rejection_reason}</span>}</div></div></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{driver.status!=='approved'?<button style={{...btn,opacity:canApprove?1:.65}} onClick={()=>decideDriver(driver,true)}>Aprovar motorista</button>:<span style={{...btn,background:'#164e33',color:'#bbf7d0',cursor:'default'}}>✓ Motorista aprovado</span>}<button style={{...btn,background:'#3a1b1b',color:'#fff'}} onClick={()=>decideDriver(driver,false)}>Reprovar motorista</button></div></div>
   <div style={{marginTop:12,padding:10,borderRadius:10,background:canApprove?'#0d2d1f':'#2b240d',color:canApprove?'#86efac':'#fde68a',fontSize:13}}><b>{canApprove?'✓ Pode aprovar agora':'⚠ Aprovação ainda não liberada'}:</b> {blockReason}</div>
   <div style={{marginTop:10,color:canApprove?'#86efac':'#d1d5db',fontSize:13}}>{p?.avatar_url?'✓ Foto presente':'• Falta foto de perfil'} · {photoApproved?'✓ Foto real conferida':'• Foto real ainda não aprovada'} · {allApproved?'✓ Todos os documentos aprovados':'• Documentação incompleta'}</div>
   <div style={{marginTop:14,display:'grid',gap:8}}>{list.length?list.map(doc=><div key={doc.id} style={{background:'#0d0d0d',border:doc.document_type==='profile_photo'?'2px solid #665600':'1px solid #272727',borderRadius:12,padding:12,display:'grid',gridTemplateColumns:'1.4fr 1fr auto',gap:12,alignItems:'center'}}><div><b>{doc.document_type==='profile_photo'?'📷 Foto real de perfil':doc.document_type}</b><div style={{color:'#9ca3af',fontSize:12}}>{new Date(doc.created_at).toLocaleString('pt-BR')}</div></div><div><span style={{padding:'5px 9px',borderRadius:999,background:'#222'}}>{doc.status}</span>{doc.rejection_reason&&<div style={{color:'#fca5a5',fontSize:12,marginTop:6}}>{doc.rejection_reason}</div>}</div><div style={{display:'flex',gap:7,flexWrap:'wrap'}}><button style={{...btn,background:'#252525',color:'#fff'}} onClick={()=>openDoc(doc)}>Ver</button>{doc.status==='pending'&&<><button style={btn} onClick={()=>review(doc,true)}>{doc.document_type==='profile_photo'?'Aprovar foto real':'Aprovar documento'}</button><button style={{...btn,background:'#3a1b1b',color:'#fff'}} onClick={()=>review(doc,false)}>Reprovar</button></>}</div></div>):<div style={{color:'#9ca3af'}}>Nenhum documento enviado. O motorista precisa concluir o cadastro no App Motorista.</div>}</div>
  </section>})}{franchiseOk&&!drivers.length&&<div style={box}><b>Nenhum motorista vinculado a esta franquia.</b><p style={{color:'#9ca3af',marginBottom:0}}>O motorista precisa escolher esta franquia/cidade no cadastro, ou a matriz pode corrigir o vínculo em Super Admin → Motoristas.</p></div>}</div>
 </main>
}
