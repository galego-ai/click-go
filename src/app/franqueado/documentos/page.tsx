...
export default function DriverDocumentsPage(){
 ...
 async function decideDriver(driver:Driver,approved:boolean){
  setMsg('');const p=names[driver.id];const list=docsByDriver[driver.id]||[]
  if(approved){if(!p?.avatar_url){setMsg('A foto de perfil real é obrigatória antes da aprovação.');return}if(!list.length){setMsg('Este motorista ainda não enviou documentos.');return}if(list.some(d=>d.status!=='approved')){setMsg('Todos os documentos precisam estar aprovados antes da liberação.');return}}
 ...
 return <main style={{minHeight:'100vh',background:'#080808',color:'#f8fafc',padding:28}}>
  <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'center',marginBottom:22}}><div><div style={{color:'#ffd400',fontWeight:800,fontSize:12,textTransform:'uppercase'}}>Central do Franqueado</div><h1 style={{margin:'5px 0'}}>Aprovação de motoristas</h1><p style={{color:'#9ca3af',margin:0}}>Confira a foto real do perfil, analise os documentos e depois aprove ou reprove o cadastro.</p></div><div style={{display:'flex',gap:8}}><Link href="/franqueado" style={{...btn,textDecoration:'none'}}>Voltar ao painel</Link><button style={btn} onClick={load}>{busy?'Atualizando...':'Atualizar'}</button></div></div>
  {msg&&<div style={{...box,borderColor:'#665600',color:'#ffe66b',marginBottom:16}}>{msg}</div>}
  <div style={{display:'grid',gap:16}}>{drivers.map(driver=>{const p=names[driver.id];const list=docsByDriver[driver.id]||[];const allApproved=list.length>0&&list.every(d=>d.status==='approved');const canApprove=!!p?.avatar_url&&allApproved;return <section key={driver.id} style={{...box,borderColor:driver.status==='approved'?'#176b45':driver.status==='rejected'?'#6b1f1f':'#4b4b22'}}>
   ...
   <div style={{marginTop:12,color:canApprove?'#86efac':'#fde68a',fontSize:13}}>{p?.avatar_url?'✓ Foto de perfil presente':'• Falta foto de perfil'} · {allApproved?'✓ Documentos aprovados':'• Existem documentos pendentes/reprovados'}</div>
