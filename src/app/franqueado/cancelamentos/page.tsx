...
export default function FranchiseCancellationPage(){
 ...
 async function savePolicy(){if(!fid)return;if(policy?.locked_by_matrix){setMsg('Este tempo foi bloqueado pela Matriz e não pode ser alterado pela franquia.');return}const n=Number(minutes);if(!Number.isFinite(n)||n<0||n>1440){setMsg('Informe um tempo entre 0 e 1440 minutos.');return}setBusy(true);const seconds=Math.round(n*60);const{error}=await supabase.from('franchise_settings').upsert({franchise_id:fid,setting_key:'passenger_cancellation_policy',setting_value:{free_seconds:seconds},source:'franchise',locked_by_matrix:false},{onConflict:'franchise_id,setting_key'});setMsg(error?error.message:`Tempo grátis atualizado para ${n} minuto${n===1?'':'s'} após o motorista iniciar o deslocamento.`);if(!error)await load();setBusy(false)}
 async function saveFee(c:Category){if(c.locked_by_matrix){setMsg('Esta categoria está bloqueada pela Matriz.');return}const value=Number(c.cancellation_fee);if(!Number.isFinite(value)||value<0){setMsg('Informe uma taxa válida.');return}setBusy(true);const{error}=await supabase.from('ride_categories').update({cancellation_fee:value}).eq('id',c.id);setMsg(error?error.message:`Taxa de ${c.name} atualizada para ${money(value)}.`);if(!error)await load();setBusy(false)}
