...
# Cada MapView deve ter seus próprios Marker objects. Reaproveitar Marker de um mapa
# já detached pode derrubar o osmdroid ao voltar da busca de endereço.
...
# Ao abrir a etapa de endereço, invalida todos os objetos visuais do mapa anterior.
...
# Nunca reutiliza marcadores de uma tela de opções anterior.
...
# Debounce simples: não interrompe threads de rede e só mexe em Views anexadas.
...
# A busca agora envia a posição atual para o backend. Isso regionaliza Mapbox/OSM/Google
# e evita resultados de cidades distantes. Toda resposta antiga é descartada.
...
# Selecionar endereço não pode derrubar a Activity. Valida coordenadas e troca a tela
# somente depois que a busca antiga foi invalidada.
...
# O cancelamento não interrompe HttpURLConnection no meio de callbacks de tela;
# apenas invalida a resposta pelo searchSeq.
pattern=r'''    private void cancelAddressSearch\(\) \{.*?\n    \}\n\n    private void cancelAddressSearchOnly\(\) \{.*?\n    \}\n\n    private void obtainLocation'''
replacement=r'''    private void cancelAddressSearch(){++searchSeq;cancelAddressSearchOnly();}

    private void cancelAddressSearchOnly(){
        if(pendingAddressSearch!=null){ui.removeCallbacks(pendingAddressSearch);pendingAddressSearch=null;}
        Future<?> current=addressFuture;addressFuture=null;
        if(current!=null&&!current.isDone())current.cancel(false);
    }

    private void obtainLocation'''
sub(pattern,replacement,'cancelamento seguro')

# Em aparelhos com pouca memória, repassa o aviso ao MapView ativo.
anchor='''    @Override protected void onDestroy() {\n'''
if anchor not in text: raise SystemExit('onDestroy não encontrado')
low='''    @Override public void onLowMemory(){\n        super.onLowMemory();\n        try{if(map!=null)map.onLowMemory();}catch(Exception ignored){}\n    }\n\n'''
text=text.replace(anchor,low+anchor,1)
...
print('Passageiro v2.1 PRIME: busca regionalizada, mapa e transições estabilizados.')
