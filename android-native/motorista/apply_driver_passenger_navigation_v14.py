...
pattern = r'''    private void renderRide\(JSONObject r\)\{.*?\}\n(?=    private void advance\(String rideId,String action\))'''
text, n = re.subn(pattern, new_render, text, count=1, flags=re.S)
if n != 1:
    raise SystemExit('renderRide final PRIME não encontrado para aplicar navegação do passageiro')
