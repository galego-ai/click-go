...
# O patch v2.3 altera somente o login. Esta etapa garante que os métodos de
# cadastro/foto já existentes continuem presentes depois da troca do método.
if 'private void showRegistration()' not in text:
...
'''
    if marker not in text:
        raise SystemExit('loadSession marker não encontrado ao restaurar cadastro')
    text = text.replace(marker, methods + marker, 1)

path.write_text(text, encoding='utf-8')
print('Motorista v2.3: cadastro e foto preservados após atualização de login.')
