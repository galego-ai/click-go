-- Índices de apoio para as novas estruturas do franchising
create index if not exists franchise_collection_rules_updated_by_idx
  on public.franchise_collection_rules(updated_by)
  where updated_by is not null;

create index if not exists franchise_invoice_adjustments_created_by_idx
  on public.franchise_invoice_adjustments(created_by)
  where created_by is not null;

create index if not exists franchise_invoice_adjustments_invoice_id_idx
  on public.franchise_invoice_adjustments(invoice_id)
  where invoice_id is not null;

-- Atribuição territorial atômica. Uma cidade pertence a uma única franquia,
-- salvo decisão explícita da Matriz, que aqui é tratada como transferência/sobrescrita.
create or replace function public.matrix_assign_franchise_city(
  p_franchise_id uuid,
  p_city_id uuid,
  p_reason text,
...
grant execute on function public.matrix_assign_franchise_city(uuid,uuid,text,boolean) to authenticated;

create or replace function public.matrix_remove_franchise_city(
  p_franchise_id uuid,
  p_city_id uuid,
  p_reason text
)
...
begin
  ...
  select name||'/'||state into v_city_name from public.cities where id=p_city_id;
  select trade_name into v_franchise_name from public.franchises where id=p_franchise_id;
...
grant execute on function public.matrix_remove_franchise_city(uuid,uuid,text) to authenticated;
