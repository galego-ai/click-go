create or replace function public.audit_direct_advertising_banner_write()
returns trigger
language plpgsql
security definer
...
begin
  if current_setting('clickgo.banner_rpc',true)='1' then
    return coalesce(new,old);
  end if;
  if v_uid is not null and v_role='super_admin' then
    insert into public.audit_logs(actor_id,action,entity,entity_id,metadata)
    values(
      v_uid,
      case tg_op when 'INSERT' then 'advertising_banner_created_direct' when 'UPDATE' then 'advertising_banner_updated_direct' else 'advertising_banner_deleted_direct' end,
      'advertising_banners',v_id::text,
      jsonb_build_object(
        'source','matrix_direct_compat',
        'franchise_id',coalesce(new.franchise_id,old.franchise_id),
...
revoke execute on function public.audit_direct_advertising_banner_write() from public,anon,authenticated;

drop trigger if exists trg_audit_direct_advertising_banner_write on public.advertising_banners;
create trigger trg_audit_direct_advertising_banner_write
after insert or update or delete on public.advertising_banners
for each row execute function public.audit_direct_advertising_banner_write();

-- Recreate RPC with a transaction-local marker to avoid duplicate audit entries.
...
begin
  ...
  perform set_config('clickgo.banner_rpc','1',true);
...
-- Matrix direct compatibility: select/create/edit only. No browser-side deletes.
drop policy if exists super_admin_banners_all on public.advertising_banners;
drop policy if exists super_admin_banners_select on public.advertising_banners;
...
create policy super_admin_banners_select on public.advertising_banners for select to authenticated using (public.current_active_management_role()='super_admin');
create policy super_admin_banners_insert on public.advertising_banners for insert to authenticated with check (public.current_active_management_role()='super_admin');
create policy super_admin_banners_update on public.advertising_banners for update to authenticated using (public.current_active_management_role()='super_admin') with check (public.current_active_management_role()='super_admin');
revoke delete,truncate,references,trigger on table public.advertising_banners from authenticated;
grant select,insert,update on table public.advertising_banners to authenticated;
