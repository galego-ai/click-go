...
-- A própria equipe enxerga a configuração de permissões do seu acesso.
drop policy if exists staff_permissions_self_select on public.franchise_staff_permissions;
create policy staff_permissions_self_select on public.franchise_staff_permissions
for select using (profile_id=auth.uid());

-- Operação/empresa e território.
drop policy if exists operator_franchise_select on public.franchises;
create policy operator_franchise_select on public.franchises
for select using (public.jwt_app_role()='operator' and id=public.staff_franchise_id());

drop policy if exists operator_franchise_cities_select on public.franchise_cities;
create policy operator_franchise_cities_select on public.franchise_cities
for select using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id());

drop policy if exists operator_profiles_scope_select on public.profiles;
create policy operator_profiles_scope_select on public.profiles
for select using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and (public.staff_has_permission('users') or public.staff_has_permission('drivers') or public.staff_has_permission('support') or public.staff_has_permission('operation')));

-- Motoristas e documentos.
drop policy if exists operator_drivers_scope_select on public.drivers;
create policy operator_drivers_scope_select on public.drivers
for select using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and (public.staff_has_permission('drivers') or public.staff_has_permission('operation') or public.staff_has_permission('support')));
drop policy if exists operator_drivers_scope_update on public.drivers;
create policy operator_drivers_scope_update on public.drivers
for update using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('drivers'))
with check (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('drivers'));

drop policy if exists operator_driver_documents_select on public.driver_documents;
create policy operator_driver_documents_select on public.driver_documents
for select using (public.jwt_app_role()='operator' and exists(select 1 from public.drivers d where d.id=driver_documents.driver_id and d.franchise_id=public.staff_franchise_id()) and (public.staff_has_permission('drivers') or public.staff_has_permission('support')));
drop policy if exists operator_driver_documents_update on public.driver_documents;
create policy operator_driver_documents_update on public.driver_documents
for update using (public.jwt_app_role()='operator' and exists(select 1 from public.drivers d where d.id=driver_documents.driver_id and d.franchise_id=public.staff_franchise_id()) and public.staff_has_permission('drivers'));

-- Corridas/dispatch.
drop policy if exists operator_rides_scope_select on public.rides;
create policy operator_rides_scope_select on public.rides
for select using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and (public.staff_has_permission('operation') or public.staff_has_permission('finance') or public.staff_has_permission('support')));
drop policy if exists operator_rides_scope_insert on public.rides;
create policy operator_rides_scope_insert on public.rides
for insert with check (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('operation'));
drop policy if exists operator_rides_scope_update on public.rides;
create policy operator_rides_scope_update on public.rides
for update using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('operation'))
with check (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('operation'));

-- Tarifas e configuração regional.
...
for select using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and (public.staff_has_permission('pricing') or public.staff_has_permission('operation')));
drop policy if exists operator_categories_write on public.ride_categories;
create policy operator_categories_write on public.ride_categories
for all using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('pricing') and not locked_by_matrix)
with check (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('pricing') and not locked_by_matrix);

drop policy if exists operator_franchise_settings_select on public.franchise_settings;
create policy operator_franchise_settings_select on public.franchise_settings
for select using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id());
drop policy if exists operator_franchise_settings_write on public.franchise_settings;
create policy operator_franchise_settings_write on public.franchise_settings
for all using (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('settings') and not locked_by_matrix)
with check (public.jwt_app_role()='operator' and franchise_id=public.staff_franchise_id() and public.staff_has_permission('settings') and not locked_by_matrix);

-- Marketing regional.
do $$
declare t text;
begin
 foreach t in array array['advertising_banners','promotions','coupons'] loop
