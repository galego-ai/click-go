...
as $$
  ...
  sampled as (
    ...
    where n.cnt <= p.max_points
       ...
       or mod(
            n.rn - 1,
            greatest(1, ceil(n.cnt::numeric / p.max_points)::integer)
          ) = 0
  ),
  aggregated as (
    select
      s.ride_id,
      jsonb_agg(
        jsonb_build_object('lat', s.lat, 'lng', s.lng)
  ...
  from mine m
  left join aggregated a on a.ride_id = m.id
...
create or replace function public.get_my_driver_earnings_history(
...
as $$
  ...
  select
    ...
    greatest(
      coalesce(r.final_fare, r.estimated_fare, 0)::numeric - coalesce(d.wallet_discount, 0)::numeric,
      0::numeric
    ) as net_earning
  from public.rides r
