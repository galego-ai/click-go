create or replace function public.list_driver_signup_categories()
returns table(
  city_id uuid,
  franchise_id uuid,
  category_id uuid,
...
as $$
  ...
  join public.cities c on c.id = rc.city_id
  join public.franchises f on f.id = rc.franchise_id
  join public.franchise_cities fc
    on fc.city_id = rc.city_id
   and fc.franchise_id = rc.franchise_id
...
revoke all on function public.list_driver_signup_categories() from public;
grant execute on function public.list_driver_signup_categories() to anon, authenticated, service_role;
...
begin
  ...
  if v_role = 'passenger' then
    ...
    v_requested_franchise := nullif(new.raw_user_meta_data->>'requested_franchise_id','')::uuid;
    v_requested_category := nullif(new.raw_user_meta_data->>'requested_category_id','')::uuid;

    if v_city is null then
      raise exception 'Escolha a cidade/franquia para cadastro';
    end if;

    if v_requested_franchise is not null then
      ...
      where fc.city_id = v_city
        and fc.franchise_id = v_requested_franchise
        and f.active = true
        and f.deleted_at is null
        and c.active = true
      limit 1;
    else
      select fc.franchise_id into v_franchise
      from public.franchise_cities fc
      join public.franchises f on f.id = fc.franchise_id
      join public.cities c on c.id = fc.city_id
      where fc.city_id = v_city
    ...
    if v_franchise is null then
      raise exception 'Franquia/cidade indisponível para cadastro';
    end if;
    ...
    if v_requested_category is not null then
      ...
      where rc.id = v_requested_category
        ...
        and rc.active = true;
      if not found then
        raise exception 'Categoria de veículo indisponível para esta franquia/cidade';
      end if;
    end if;
