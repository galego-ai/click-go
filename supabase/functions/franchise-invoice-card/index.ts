...
Deno.serve(async(req)=>{
 try{
  ...
  if(action==="create"&&pixActive)return json({error:"Esta fatura já possui um Pix ativo. Pague ou aguarde a expiração do Pix antes de gerar cobrança por cartão.",conflict:"pix",pix:pixActive},409);
  ...
  if(!clientId||!clientSecret)return json({error:"API de Emissão de cobranças da Efí não configurada",missing:[!clientId?"EFI_BILLING_CLIENT_ID/EFI_CLIENT_ID":null,!clientSecret?"EFI_BILLING_CLIENT_SECRET/EFI_CLIENT_SECRET":null].filter(Boolean)},503);
  ...
  const syncCharge=async(charge:CardRow)=>{
   ...
   if(!res.ok){await admin.from("franchise_invoice_card_charges").update({provider_status:`HTTP_${res.status}`,raw_response:body,updated_at:new Date().toISOString()}).eq("id",charge.id);return {error:true,status:res.status,provider:body};}
   ...
   else if(providerStatus==="unpaid")local="unpaid";
   const paidAt=data?.payment?.paid_at||((local==="paid")?new Date().toISOString():null);
   const paymentUrl=data?.payment_url||data?.link||charge.payment_url;
   const updated=await admin.from("franchise_invoice_card_charges").update({status:local,provider_status:providerStatus||charge.provider_status,payment_url:paymentUrl,paid_at:paidAt,raw_response:body,updated_at:new Date().toISOString()}).eq("id",charge.id).select("*").single();
   let paymentResult=null;
   if(local==="paid"&&charge.status!=="paid")paymentResult=await markPaid(charge,paidAt);
  ...
  const expireAt=invoice.due_date&&invoice.due_date>=today?invoice.due_date:addDays(today,1);
  const payload={
   items:[{name:`Fatura CLICK-GO ${reference.slice(0,7)}`,value:Math.max(1,Math.round(total*100)),amount:1}],
   metadata:{custom_id:`CLICKGO-${invoice.id}`},
   customer:{email},
   settings:{payment_method:"credit_card",expire_at:expireAt,request_delivery_address:false,message:`Fatura CLICK-GO ${reference.slice(0,7)}`}
  };
  const createRes=await fetch(`${base}/v1/charge/one-step/link`,{method:"POST",headers,body:JSON.stringify(payload)});
  ...
  const chargePayload={invoice_id:invoice.id,franchise_id:franchiseId,created_by:user.id,provider:"efi",charge_id:Number(data.charge_id),payment_url:String(data.payment_url),amount:total,status:"active",provider_status:String(data.status||"link").toLowerCase(),raw_response:created,updated_at:new Date().toISOString()};
  const inserted=await admin.from("franchise_invoice_card_charges").insert(chargePayload).select("*").single();
