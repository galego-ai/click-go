...
Deno.serve(async(req)=>{
 try{
  ...
  if(action==="create"){
   const {data:activeCard}=await admin.from("franchise_invoice_card_charges").select("id,charge_id,status,payment_url").eq("invoice_id",invoice.id).eq("status","active").order("created_at",{ascending:false}).limit(1).maybeSingle();
   if(activeCard)return json({error:"Esta fatura já possui uma cobrança por cartão ativa. Use essa cobrança antes de gerar um Pix.",conflict:"card",card:activeCard},409);
  }
  ...
  const markPaid=async(charge:ChargeRow,paidAt:string|null,endToEnd:string|null)=>{
   const marked=await admin.rpc("mark_franchise_invoice_paid",{p_invoice_id:invoice.id,p_paid_at:paidAt,p_actor_id:user.id,p_source:role==="super_admin"?"matrix":"franchise",p_method:"pix",p_provider_ref:endToEnd||charge.txid});
  ...
  const syncCharge=async(charge:ChargeRow)=>{
   ...
   if(firstPaid)paymentResult=await markPaid(charge,paidAt,endToEnd);
   return {error:false,charge:updated.data||{...charge,...update},provider_status:providerStatus,paid:local==="paid",payment_result:paymentResult};
  };
  ...
  if(existing&&existing.status==="active"){
   if(Math.abs(money(existing.amount)-total)>0.009)return json({error:"A fatura mudou depois da geração do Pix. Aguarde a cobrança atual encerrar antes de gerar outra.",charge:existing,summary},409);
   const synced=await syncCharge(existing as ChargeRow);
  ...
  const now=Date.now();
  const dueTime=invoice.due_date?Date.parse(`${invoice.due_date}T23:59:59-03:00`):now+7*86400000;
